...
SearchPluginManager::SearchPluginManager()
    ...
    connect(Net::ProxyConfigurationManager::instance(), &Net::ProxyConfigurationManager::proxyConfigurationChanged, this,
            &SearchPluginManager::applyProxySettings);
    connect(Preferences::instance(), &Preferences::changed, this, &SearchPluginManager::applyProxySettings);
    applyProxySettings();
...
void SearchPluginManager::installPlugin(const QString &source) {
    ...
    if (Net::DownloadManager::hasSupportedScheme(source)) {
        using namespace Net;
        DownloadManager::instance()->download(DownloadRequest(source).saveToFile(true), Preferences::instance()->useProxyForGeneralPurposes(), this,
                                              &SearchPluginManager::pluginDownloadFinished);
    } else {
...
void SearchPluginManager::checkForUpdates() {
    ...
    using namespace Net;
    DownloadManager::instance()->download({m_updateUrl + u"versions.txt"}, Preferences::instance()->useProxyForGeneralPurposes(), this,
                                          &SearchPluginManager::versionInfoDownloadFinished);
}
...
void SearchPluginManager::applyProxySettings() {
    ...
    if ((proxyConfig.type != Net::ProxyType::None) && Preferences::instance()->useProxyForGeneralPurposes()) {
        switch (proxyConfig.type) {
            case Net::ProxyType::HTTP:
                if (proxyConfig.authEnabled) {
                    proxyStrHTTP = u"http://%1:%2@%3:%4"_s.arg(proxyConfig.username, proxyConfig.password, proxyConfig.ip, QString::number(proxyConfig.port));
                } else {
            ...
            case Net::ProxyType::SOCKS5:
                if (proxyConfig.authEnabled) {
                    proxyStrSOCK = u"%1:%2@%3:%4"_s.arg(proxyConfig.username, proxyConfig.password, proxyConfig.ip, QString::number(proxyConfig.port));
                } else {
        ...
        qDebug("HTTP communications proxy string: %s", qUtf8Printable((proxyConfig.type == Net::ProxyType::SOCKS5) ? proxyStrSOCK : proxyStrHTTP));
    }
