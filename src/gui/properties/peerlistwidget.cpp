...
namespace {
    void setModelData(QStandardItemModel *model, const int row, const int column, const QString &displayData, const QVariant &underlyingData,
                      const Qt::Alignment textAlignmentData = {}, const QString &toolTip = {}) {
...
PeerListWidget::PeerListWidget(PropertiesWidget *parent)
        : QTreeView(parent), m_properties(parent) {
    // Load settings
    ...
    m_listModel->setHeaderData(PeerListColumns::RELEVANCE, Qt::Horizontal,
...
void PeerListWidget::showPeerListMenu() {
    ...
    QAction *addNewPeer = menu->addAction(UIThemeManager::instance()->getIcon(u"peers-add"_s), tr("Add peers..."), this, [this, torrent]() {
    ...
    QAction *copyPeers = menu->addAction(UIThemeManager::instance()->getIcon(u"edit-copy"_s), tr("Copy IP:port"), this, &PeerListWidget::copySelectedPeers);
    menu->addSeparator();
    QAction *banPeers = menu->addAction(UIThemeManager::instance()->getIcon(u"peers-remove"_s), tr("Ban peer permanently"), this,
                                        &PeerListWidget::banSelectedPeers);
...
void PeerListWidget::updatePeer(const int row, const BitTorrent::Torrent *torrent, const BitTorrent::PeerInfo &peer, const bool hideZeroValues) {
    ...
    setModelData(m_listModel, row, PeerListColumns::PROGRESS, (Utils::String::fromDouble(peer.progress() * 100, 1) + u'%'), peer.progress(),
                 intDataTextAlignment);
    setModelData(m_listModel, row, PeerListColumns::RELEVANCE, (Utils::String::fromDouble(peer.relevance() * 100, 1) + u'%'), peer.relevance(),
                 intDataTextAlignment);
    ...
    setModelData(m_listModel, row, PeerListColumns::DOWNLOADING_PIECE, downloadingFilesDisplayValue, downloadingFilesDisplayValue, {},
                 downloadingFiles.join(u'\n'));
